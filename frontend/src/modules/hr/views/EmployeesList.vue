    ...
    <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div class="flex items-start gap-4 mb-4">
          <div class="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground font-semibold text-lg">
            {{ getInitials(employee.fullName) }}
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <h3 class="font-semibold truncate">{{ employee.fullName }}</h3>
              <span :class="cn('text-xs px-2 py-0.5 rounded-full', getStatusColor(employee.status))">
        ...
        <div class="space-y-2 text-sm">
          ...
          <div v-if="employee.phone" class="flex items-center gap-2 text-muted-foreground">
        ...
        <!-- Manager -->
        <div v-if="employee.managerName" class="mt-4 pt-4 border-t">
