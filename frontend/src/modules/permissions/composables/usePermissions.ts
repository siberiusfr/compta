...
export function usePermissions() {
  ...
  // Utilise le nouveau composable useRoles avec Vue Query
  const { roles, isLoading: isLoadingRoles, roleCount } = useRoles()

  // Garde le store pour les groupes et permissions (mock pour l'instant)
  const store = usePermissionsStore()
  const { groups, permissions, isLoading: isLoadingStore, groupCount } = storeToRefs(store)

  // Combine les etats de chargement
  const isLoading = computed(
    () => isLoadingUsers.value || isLoadingRoles.value || isLoadingStore.value
  )
