...
public class UserSocietesService {
    ...
    public UserSocietesDto assignUser(AssignUserToSocieteRequest request) {
        ...
        if (userSocietesRepository.existsByUserId(request.getUserId())) {
            throw new IllegalArgumentException(
                    "L'utilisateur est deja assigne a une societe cliente");
        }
        ...
        log.info("Utilisateur {} assigne a la societe {} avec le rôle {}",
                request.getUserId(), request.getSocieteId(), role.getCode());
    ...
    public UserSocietesDto updateRole(Long id, String newRole) {
        ...
        UserSocietes existing = userSocietesRepository.findById(id)
    ...
    public UserSocietesDto findById(Long id) {
        return userSocietesRepository.findById(id)
                .map(this::toDto)
                .orElseThrow(() -> new IllegalArgumentException("Assignation non trouvee: " + id));
    }
    ...
    public UserSocietesDto findByUserId(Long userId) {
        return userSocietesRepository.findByUserId(userId)
                .map(this::toDto)
    ...
    public UserSocietesDto findActiveByUserId(Long userId) {
        return userSocietesRepository.findActiveByUserId(userId)
                .map(this::toDto)
                .orElse(null);
    }
    ...
    public List<UserSocietesDto> findBySocieteId(Long societeId) {
        return userSocietesRepository.findBySocieteId(societeId).stream()
                .map(this::toDto)
                .toList();
    }
    ...
    public UserSocietesDto findManagerBySocieteId(Long societeId) {
        return userSocietesRepository.findManagerBySocieteId(societeId)
                .map(this::toDto)
                .orElse(null);
    }
    ...
    public SocieteDto findSocieteByUserId(Long userId) {
        return userSocietesRepository.findSocieteByUserId(userId)
