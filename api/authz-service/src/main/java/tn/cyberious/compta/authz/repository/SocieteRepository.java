...
public class SocieteRepository {
  ...
  public boolean delete(Long id) {
    return dsl.deleteFrom(SOCIETES).where(SOCIETES.ID.eq(id)).execute() > 0;
  }
  ...
  public List<Societes> findAll() {
    return dsl.selectFrom(SOCIETES).orderBy(SOCIETES.RAISON_SOCIALE).fetch().into(Societes.class);
  }
  ...
  public boolean existsById(Long id) {
    return dsl.fetchExists(dsl.selectOne().from(SOCIETES).where(SOCIETES.ID.eq(id)));
  }

  public boolean existsByMatriculeFiscale(String matriculeFiscale) {
    return dsl.fetchExists(
        dsl.selectOne().from(SOCIETES).where(SOCIETES.MATRICULE_FISCALE.eq(matriculeFiscale)));
  }
}
