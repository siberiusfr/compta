...
@Tag(name = "Utilisateurs Cabinet", description = "Gestion des utilisateurs appartenant aux cabinets comptables")
public class UserSocieteComptableController {

    private final UserSocieteComptableService userSocieteComptableService;

    @PostMapping
    @Operation(summary = "Assigner un utilisateur a un cabinet", description = "Assigne un utilisateur a une societe comptable avec un rôle")
    @ApiResponses({
    ...
    @PutMapping("/{id}/role")
    @Operation(summary = "Modifier le rôle d'un utilisateur", description = "Change le rôle d'un utilisateur dans son cabinet")
    @ApiResponses({
    ...
    public ResponseEntity<UserSocieteComptableDto> updateRole(
            ...
            @Parameter(description = "Nouveau rôle (MANAGER, COMPTABLE, ASSISTANT)") @RequestParam String role) {
        return ResponseEntity.ok(userSocieteComptableService.updateRole(id, role));
    }

    @PutMapping("/{id}/deactivate")
    @Operation(summary = "Desactiver une assignation", description = "Desactive l'assignation d'un utilisateur a son cabinet")
    @ApiResponses({
    ...
    @Operation(summary = "Recuperer par utilisateur", description = "Recupere l'assignation d'un utilisateur a son cabinet")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Assignation trouvee"),
    ...
    @Operation(summary = "Lister par cabinet", description = "Recupere tous les utilisateurs d'un cabinet comptable")
    ...
    public ResponseEntity<List<UserSocieteComptableDto>> findBySocieteComptableId(
        ...
        return ResponseEntity.ok(userSocieteComptableService.findBySocieteComptableId(societeComptableId));
    }

    @GetMapping("/cabinet/{societeComptableId}/active")
    @Operation(summary = "Lister les actifs par cabinet", description = "Recupere les utilisateurs actifs d'un cabinet comptable")
    ...
    public ResponseEntity<List<UserSocieteComptableDto>> findActiveBySocieteComptableId(
            @Parameter(description = "ID de la societe comptable") @PathVariable Long societeComptableId) {
        return ResponseEntity.ok(userSocieteComptableService.findActiveBySocieteComptableId(societeComptableId));
    }

    @GetMapping("/cabinet/{societeComptableId}/manager")
    @Operation(summary = "Recuperer le manager", description = "Recupere le manager d'un cabinet comptable")
    ...
    public ResponseEntity<UserSocieteComptableDto> findManagerBySocieteComptableId(
            @Parameter(description = "ID de la societe comptable") @PathVariable Long societeComptableId) {
        return ResponseEntity.ok(userSocieteComptableService.findManagerBySocieteComptableId(societeComptableId));
    }
}
