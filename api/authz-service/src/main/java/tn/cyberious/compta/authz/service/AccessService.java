...
public class AccessService {
  ...
  /**
   * Verifie si un utilisateur a acces a une societe. L'acces peut etre: - Via comptable_societes
   * (comptable avec acces a la societe cliente) - Via user_societes (employe/manager de la societe)
   */
  ...
  public UserAccessDto getUserAccess(Long userId, Long societeId) {
    ...
    Optional<String> memberRole =
        userSocietesRepository.findRoleByUserIdAndSocieteId(userId, societeId);
    if (memberRole.isPresent()) {
      log.debug(
          "Utilisateur {} est membre de la societe {} avec role {}",
          userId,
          societeId,
          memberRole.get());
  ...
  @Transactional(readOnly = true)
  @Cacheable(
      value = CacheConfig.USER_ACCESS_CACHE,
      key = "'validate-' + #userId + '-' + #societeId")
  ...
  /**
   * Recupere toutes les societes auxquelles un utilisateur a acces. Combine les acces via
   * comptable_societes et user_societes.
   */
  @Transactional(readOnly = true)
  ...
  public List<SocieteAccessDto> getAccessibleSocietes(Long userId) {
    ...
    userSocietesRepository
        .findSocieteByUserId(userId)
        .ifPresent(
            societe -> {
              String role =
                  userSocietesRepository
                      .findRoleByUserIdAndSocieteId(userId, societe.getId())
                      .orElse("VIEWER");
              result.add(
                  SocieteAccessDto.builder()
    ...
    List<ComptableSocietes> comptableAccesses =
        comptableSocietesRepository.findActiveByUserId(userId);
    for (ComptableSocietes access : comptableAccesses) {
      // Eviter les doublons si l'utilisateur est aussi membre
      boolean alreadyAdded =
          result.stream().anyMatch(s -> s.getSocieteId().equals(access.getSocieteId()));
      if (!alreadyAdded) {
        // Recuperer les infos de la societe
        societeRepository
            .findById(access.getSocieteId())
            .ifPresent(
                societe -> {
                  String comptableRole =
                      userSocieteComptableRepository
