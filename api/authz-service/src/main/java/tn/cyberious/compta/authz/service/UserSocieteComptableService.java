...
public class UserSocieteComptableService {
    ...
    public UserSocieteComptableDto assignUser(AssignUserToSocieteComptableRequest request) {
        ...
        if (userSocieteComptableRepository.existsByUserId(request.getUserId())) {
            throw new IllegalArgumentException(
                    "L'utilisateur est deja assigne a une societe comptable");
        }
        ...
        if (role == CabinetRole.MANAGER
            ...
            throw new IllegalArgumentException(
                    "Cette societe comptable a deja un manager actif");
        }
        ...
        log.info("Utilisateur {} assigne a la societe comptable {} avec le rôle {}",
                request.getUserId(), request.getSocieteComptableId(), role.getCode());
    ...
    public UserSocieteComptableDto updateRole(Long id, String newRole) {
        ...
        UserSocieteComptable existing = userSocieteComptableRepository.findById(id)
    ...
    public UserSocieteComptableDto findById(Long id) {
        return userSocieteComptableRepository.findById(id)
                .map(this::toDto)
                .orElseThrow(() -> new IllegalArgumentException("Assignation non trouvee: " + id));
    }
    ...
    public UserSocieteComptableDto findByUserId(Long userId) {
        return userSocieteComptableRepository.findByUserId(userId)
                .map(this::toDto)
    ...
    public UserSocieteComptableDto findActiveByUserId(Long userId) {
        return userSocieteComptableRepository.findActiveByUserId(userId)
                .map(this::toDto)
                .orElse(null);
    }
    ...
    public List<UserSocieteComptableDto> findActiveBySocieteComptableId(Long societeComptableId) {
        return userSocieteComptableRepository.findActiveBySocieteComptableId(societeComptableId).stream()
                .map(this::toDto)
                .toList();
    }
    ...
    public UserSocieteComptableDto findManagerBySocieteComptableId(Long societeComptableId) {
        return userSocieteComptableRepository.findManagerBySocieteComptableId(societeComptableId)
                .map(this::toDto)
                .orElse(null);
    }
