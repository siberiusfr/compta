...
public class SocieteComptableService {
    ...
    public SocieteComptableDto create(CreateSocieteComptableRequest request) {
        if (societeComptableRepository.existsByMatriculeFiscale(request.getMatriculeFiscale())) {
            throw new IllegalArgumentException(
                    "Une societe comptable avec ce matricule fiscale existe deja: " + request.getMatriculeFiscale());
        }
    ...
    public SocieteComptableDto update(Long id, UpdateSocieteComptableRequest request) {
        SocietesComptables existing = societeComptableRepository.findById(id)
        ...
        log.info("Societe comptable mise a jour: {} (ID: {})", updated.getRaisonSociale(), updated.getId());
    ...
    public void delete(Long id) {
        ...
        if (societeCount > 0) {
            throw new IllegalStateException(
                    "Impossible de supprimer: cette societe comptable gere " + societeCount + " societe(s) cliente(s)");
        }
    ...
    public SocieteComptableDto findById(Long id) {
        return societeComptableRepository.findById(id)
                .map(this::toDto)
                .orElseThrow(() -> new IllegalArgumentException("Societe comptable non trouvee: " + id));
    }
    ...
    public SocieteComptableDto findByMatriculeFiscale(String matriculeFiscale) {
        return societeComptableRepository.findByMatriculeFiscale(matriculeFiscale)
                .map(this::toDto)
    ...
    public List<SocieteComptableDto> findAll() {
        return societeComptableRepository.findAll().stream()
                .map(this::toDto)
                .toList();
    }
    ...
    public List<SocieteComptableDto> findAllActive() {
        return societeComptableRepository.findAllActive().stream()
                .map(this::toDto)
                .toList();
    }
