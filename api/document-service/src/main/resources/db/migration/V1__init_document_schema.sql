...
CREATE OR REPLACE FUNCTION document.update_updated_at_column () RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP;
...
UPDATE ON document.categories FOR EACH ROW
EXECUTE FUNCTION document.update_updated_at_column ();

CREATE TRIGGER update_documents_updated_at BEFORE
UPDATE ON document.documents FOR EACH ROW
EXECUTE FUNCTION document.update_updated_at_column ();

-- Insertion des catégories par défaut
INSERT INTO
  document.categories (name, description)
VALUES
  (
    'Comptabilité',
