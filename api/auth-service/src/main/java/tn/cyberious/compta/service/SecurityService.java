...
public class SecurityService {
  ...
  public void checkSocieteAccess(CustomUserDetails currentUser, Long societeId) {
    ...
    if (roles.contains(Role.COMPTABLE)) {
      List<Long> accessibleSocietes =
          societeRepository.findSocieteIdsByComptableId(currentUser.getId());

      if (!accessibleSocietes.contains(societeId)) {
        log.warn(
            "Comptable {} attempted to access societe {} without permission",
            currentUser.getId(),
            societeId);
    ...
    if (roles.contains(Role.SOCIETE)) {
      List<Long> accessibleSocietes =
          societeRepository.findSocieteIdsByUserSocieteId(currentUser.getId());

      if (!accessibleSocietes.contains(societeId)) {
        log.warn(
            "User societe {} attempted to access societe {} without permission",
            currentUser.getId(),
            societeId);
        throw new ForbiddenException("Vous n'avez pas accès à cette société");
      }
    ...
    if (roles.contains(Role.EMPLOYEE)) {
      List<Long> accessibleSocietes =
          societeRepository.findSocieteIdsByEmployeeId(currentUser.getId());

      if (!accessibleSocietes.contains(societeId)) {
        log.warn(
            "Employee {} attempted to access societe {} without permission",
            currentUser.getId(),
            societeId);
        throw new ForbiddenException("Vous n'avez pas accès à cette société");
      }
  ...
  public void checkSocieteWriteAccess(CustomUserDetails currentUser, Long societeId) {
    ...
    if (roles.contains(Role.COMPTABLE)) {
      ...
      if (!accessibleSocietes.contains(societeId)) {
        log.warn(
            "Comptable {} attempted to modify societe {} without permission",
            currentUser.getId(),
            societeId);
  ...
  /**
   * Vérifie si l'utilisateur courant a accès à un autre utilisateur (basé sur les sociétés
   * communes)
   */
  public void checkUserAccess(CustomUserDetails currentUser, Long targetUserId) {
    ...
    if (roles.contains(Role.COMPTABLE)) {
      ...
      boolean hasCommonSociete = comptableSocietes.stream().anyMatch(targetUserSocietes::contains);

      if (!hasCommonSociete) {
        // Vérifier aussi si c'est un employé d'une des sociétés du comptable (requête directe)
        List<Long> targetEmployeeSocietes =
            societeRepository.findSocieteIdsByEmployeeId(targetUserId);

        boolean isEmployeeInComptableSocietes =
            comptableSocietes.stream().anyMatch(targetEmployeeSocietes::contains);

        if (!isEmployeeInComptableSocietes) {
          log.warn(
              "Comptable {} attempted to access user {} without common societe",
              currentUser.getId(),
              targetUserId);
