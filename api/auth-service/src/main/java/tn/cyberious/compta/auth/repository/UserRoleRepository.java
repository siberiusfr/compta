...
public class UserRoleRepository {
  ...
  public UserRoles insert(UserRoles userRole) {
    log.debug(
        "Inserting user role: userId={}, roleId={}", userRole.getUserId(), userRole.getRoleId());

    UserRolesRecord record =
  ...
  public boolean delete(Long id) {
    ...
    int deleted = dsl.deleteFrom(USER_ROLES).where(USER_ROLES.ID.eq(id)).execute();
    return deleted > 0;
  }
  ...
  public List<UserRoles> findAll() {
    ...
    return dsl.selectFrom(USER_ROLES).fetch().into(UserRoles.class);
  }
  ...
  public boolean exists(Long id) {
    ...
    return dsl.fetchExists(dsl.selectFrom(USER_ROLES).where(USER_ROLES.ID.eq(id)));
  }
