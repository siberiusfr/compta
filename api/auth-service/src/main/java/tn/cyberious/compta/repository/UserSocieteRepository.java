...
public class UserSocieteRepository {
    ...
    public UserSocietes insert(UserSocietes userSociete) {
        log.debug("Inserting user-societe association: userId={}, societeId={}",
                userSociete.getUserId(), userSociete.getSocieteId());

        UserSocietesRecord record = dsl.insertInto(USER_SOCIETES)
                .set(USER_SOCIETES.USER_ID, userSociete.getUserId())
                .set(USER_SOCIETES.SOCIETE_ID, userSociete.getSocieteId())
                .set(USER_SOCIETES.IS_OWNER, userSociete.getIsOwner() != null ? userSociete.getIsOwner() : false)
                .set(USER_SOCIETES.DATE_DEBUT, userSociete.getDateDebut())
                .set(USER_SOCIETES.DATE_FIN, userSociete.getDateFin())
                .set(USER_SOCIETES.IS_ACTIVE, userSociete.getIsActive() != null ? userSociete.getIsActive() : true)
    ...
    public UserSocietes update(UserSocietes userSociete) {
        ...
        UserSocietesRecord record = dsl.update(USER_SOCIETES)
                .set(USER_SOCIETES.USER_ID, userSociete.getUserId())
                .set(USER_SOCIETES.SOCIETE_ID, userSociete.getSocieteId())
    ...
    public boolean delete(Long id) {
        ...
        int deleted = dsl.deleteFrom(USER_SOCIETES)
    ...
    public List<Societes> findSocietesByUserId(Long userId) {
        ...
        return dsl.select(SOCIETES.fields())
                .from(USER_SOCIETES)
                .join(SOCIETES).on(USER_SOCIETES.SOCIETE_ID.eq(SOCIETES.ID))
                .where(USER_SOCIETES.USER_ID.eq(userId))
                .fetch()
                .into(Societes.class);
    }
    ...
    public List<Users> findUsersBySocieteId(Long societeId) {
        ...
        return dsl.select(USERS.fields())
                .from(USER_SOCIETES)
                .join(USERS).on(USER_SOCIETES.USER_ID.eq(USERS.ID))
                .where(USER_SOCIETES.SOCIETE_ID.eq(societeId))
                .fetch()
                .into(Users.class);
    }

    public void assignUserToSociete(Long userId, Long societeId, Boolean isOwner, LocalDate dateDebut, LocalDate dateFin) {
