...
import tn.cyberious.compta.dto.AuthLogResponse;
import tn.cyberious.compta.service.AuthLogService;
...
public class AuthLogController {
    ...
    @Operation(summary = "Get all auth logs", description = "Get all authentication logs (ADMIN only)")
    ...
    @Operation(summary = "Get user auth logs", description = "Get authentication logs for a specific user")
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Get logs by action", description = "Get authentication logs filtered by action type (ADMIN only)")
