...
public class ComptableSocieteRepository {
  ...
  public boolean delete(Long id) {
    ...
    int deleted = dsl.deleteFrom(COMPTABLE_SOCIETES).where(COMPTABLE_SOCIETES.ID.eq(id)).execute();
    return deleted > 0;
  }
  ...
  public List<ComptableSocietes> findAll() {
    ...
    return dsl.selectFrom(COMPTABLE_SOCIETES).fetch().into(ComptableSocietes.class);
  }

  public boolean exists(Long id) {
    log.debug("Checking if comptable-societe association exists: {}", id);
    return dsl.fetchExists(dsl.selectFrom(COMPTABLE_SOCIETES).where(COMPTABLE_SOCIETES.ID.eq(id)));
  }
  ...
  public void removeComptableFromSociete(Long userId, Long societeId) {
    ...
    dsl.deleteFrom(COMPTABLE_SOCIETES)
        .where(
            COMPTABLE_SOCIETES.USER_ID.eq(userId).and(COMPTABLE_SOCIETES.SOCIETE_ID.eq(societeId)))
        .execute();
  }
}
