...
public class AuthLogRepository {
  ...
  public AuthLogs insert(AuthLogs authLog) {
    log.debug(
        "Inserting auth log: action={}, username={}", authLog.getAction(), authLog.getUsername());

    AuthLogsRecord record =
  ...
  public boolean delete(Long id) {
    ...
    int deleted = dsl.deleteFrom(AUTH_LOGS).where(AUTH_LOGS.ID.eq(id)).execute();
    return deleted > 0;
  }
  ...
  public boolean exists(Long id) {
    ...
    return dsl.fetchExists(dsl.selectFrom(AUTH_LOGS).where(AUTH_LOGS.ID.eq(id)));
  }
