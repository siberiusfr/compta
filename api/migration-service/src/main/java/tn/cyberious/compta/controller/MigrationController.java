...
/**
 * Contrôleur pour consulter l'état des migrations de base de données. Ce service ne gère plus
 * l'exécution des migrations, seulement la consultation.
 */
@RestController
...
public class MigrationController {
  ...
  @GetMapping
  @Operation(
      summary = "Liste toutes les migrations",
      description = "Retourne la liste de toutes les migrations (appliquées et en attente)")
  public ResponseEntity<List<Map<String, Object>>> getAllMigrations() {
    ...
    List<Map<String, Object>> migrationList =
        Arrays.stream(migrations).map(this::migrationInfoToMap).collect(Collectors.toList());
  ...
  @GetMapping("/applied")
  @Operation(
      summary = "Liste les migrations appliquées",
      description = "Retourne uniquement les migrations qui ont été appliquées")
  ...
  @GetMapping("/pending")
  @Operation(
      summary = "Liste les migrations en attente",
      description = "Retourne les migrations qui n'ont pas encore été appliquées")
  ...
  @GetMapping("/info")
  @Operation(
      summary = "Informations sur l'état des migrations",
      description = "Retourne des informations générales sur l'état de la base de données")
            ...
            .filter(
                m ->
                    m.getState() == MigrationState.FAILED
                        || m.getState() == MigrationState.FUTURE_FAILED)
            .count();

    Map<String, Object> info = new HashMap<>();
    info.put(
        "current",
        infoService.current() != null ? migrationInfoToMap(infoService.current()) : null);
  ...
  @GetMapping("/failed")
  @Operation(
      summary = "Liste les migrations échouées",
      description = "Retourne les migrations qui ont échoué lors de leur application")
