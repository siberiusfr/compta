...
CREATE OR REPLACE FUNCTION notification.update_updated_at_column () RETURNS TRIGGER AS $$
BEGIN
...
UPDATE ON notification.templates FOR EACH ROW
EXECUTE FUNCTION notification.update_updated_at_column ();

CREATE TRIGGER update_notifications_updated_at BEFORE
UPDATE ON notification.notifications FOR EACH ROW
EXECUTE FUNCTION notification.update_updated_at_column ();

CREATE TRIGGER update_user_preferences_updated_at BEFORE
UPDATE ON notification.user_preferences FOR EACH ROW
EXECUTE FUNCTION notification.update_updated_at_column ();

CREATE TRIGGER update_webhooks_updated_at BEFORE
UPDATE ON notification.webhooks FOR EACH ROW
EXECUTE FUNCTION notification.update_updated_at_column ();

-- Insertion des templates par défaut
INSERT INTO
  notification.templates (name, type, subject, body, variables)
VALUES
  (
    'welcome_email',
    'EMAIL',
    'Bienvenue sur Compta',
    'Bonjour {{name}}, bienvenue sur notre plateforme!',
    '["name"]'::jsonb
  ),
  (
    'password_reset',
  ...
  (
    ...
    'Une nouvelle facture #{{invoice_number}} a été créée.',
